...
class MksServo:
    ...
    def __init__(self, bus: can.interface.Bus, notifier: can.Notifier, id:int):
        ...
        def monitor_incomming_messages(message):
            try:
                if message.arbitration_id == self.can_id:
                    ...
                    if (
                        message.data[0] == MksCommands.MOTOR_CALIBRATION_COMMAND.value
                        and len(message.data) == self.GENERIC_RESPONSE_LENGTH
                    ):
                        status_int = int.from_bytes(message.data[1:2], byteorder="big")
                        try:
                            self._calibration_status = self.CalibrationResult(
                                status_int
                            )
                        except ValueError:
                        ...
                        message.data[0]
                        == MksCommands.RUN_MOTOR_RELATIVE_MOTION_BY_PULSES_COMMAND.value
                        and len(message.data) == self.GENERIC_RESPONSE_LENGTH
                    ):
                        status_int = int.from_bytes(message.data[1:2], byteorder="big")
                        try:
                        ...
                        message.data[0]
                        == MksCommands.RUN_MOTOR_ABSOLUTE_MOTION_BY_PULSES_COMMAND.value
                        and len(message.data) == self.GENERIC_RESPONSE_LENGTH
                    ):
                        status_int = int.from_bytes(message.data[1:2], byteorder="big")
                        try:
                        ...
                        message.data[0]
                        == MksCommands.RUN_MOTOR_RELATIVE_MOTION_BY_AXIS_COMMAND.value
                        and len(message.data) == self.GENERIC_RESPONSE_LENGTH
                    ):
                        status_int = int.from_bytes(message.data[1:2], byteorder="big")
                        try:
                        ...
                        message.data[0]
                        == MksCommands.RUN_MOTOR_ABSOLUTE_MOTION_BY_AXIS_COMMAND.value
                        and len(message.data) == self.GENERIC_RESPONSE_LENGTH
                    ):
                        status_int = int.from_bytes(message.data[1:2], byteorder="big")
                        try:
                        ...
                        message.data[0] == MksCommands.GO_HOME_COMMAND.value
                        and len(message.data) == self.GENERIC_RESPONSE_LENGTH
                    ):
                        status_int = int.from_bytes(message.data[1:2], byteorder="big")
                        try:
                        ...
                        message.data[0] == MksCommands.QUERY_MOTOR_STATUS_COMMAND.value
                    ):
                        a = 1
                    elif (
                        message.data[0] == MksCommands.READ_ENCODED_VALUE_ADDITION.value
                    ):
                        a = 1
                    else:
    ...
    def create_can_msg(self, msg):
        ...
        can_message = can.Message(
            arbitration_id=self.can_id, data=write_data, is_extended_id=False
        )
        ...
        def receive_message(message):
            ...
            if not status:
                try:
                    ...
                    if message.arbitration_id == self.can_id:
                        if not (
                            message.data[0] == op_code
                            and len(message.data) == response_length
                        ):
